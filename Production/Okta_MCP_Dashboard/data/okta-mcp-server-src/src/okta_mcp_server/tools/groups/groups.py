...
def _group_to_dict(g) -> dict:
    return {
        "id": g.id,
        "name": g.profile.name if g.profile else None,
        "description": g.profile.description if g.profile else None,
        "type": _safe(g.type),
        "created": _safe(g.created),
        "lastUpdated": _safe(g.last_updated),
    }


_PROFILE_FIELDS = ("login", "email", "firstName", "lastName", "displayName",
                   "mobilePhone", "secondEmail", "department", "title", "userType")


def _group_user_to_dict(u) -> dict:
    p = u.profile
    profile = {f: getattr(p, f, None) for f in _PROFILE_FIELDS} if p else {}
    return {
        **{k: v for k, v in profile.items() if v is not None},
        "id": u.id,
        "status": _safe(u.status),
        "created": _safe(u.created),
        "lastLogin": _safe(u.last_login),
    }
...
async def list_groups(
    ...
    try:
        ...
        group_items = [_group_to_dict(g) for g in groups]
        ...
        if fetch_all and response and hasattr(response, "has_next") and response.has_next():
            ...
            all_group_items = [_group_to_dict(g) for g in all_groups]

            logger.info(
                f"Successfully retrieved {len(all_group_items)} groups across {pagination_info['pages_fetched']} pages"
            )
            return create_paginated_response(
                all_group_items, response, fetch_all_used=True, pagination_info=pagination_info
            )
        else:
            logger.info(f"Successfully retrieved {len(group_items)} groups")
            return create_paginated_response(group_items, response, fetch_all_used=fetch_all)
...
async def list_group_users(
    ...
    try:
        ...
        user_items = [_group_user_to_dict(u) for u in users]
        ...
        if fetch_all and response and hasattr(response, "has_next") and response.has_next():
            ...
            all_user_items = [_group_user_to_dict(u) for u in all_users]

            pages_fetched = pagination_info["pages_fetched"]
            logger.info(
                f"Successfully retrieved {len(all_user_items)} users from group {group_id} across {pages_fetched} pages"
            )
            return create_paginated_response(all_user_items, response, fetch_all_used=True, pagination_info=pagination_info)
        else:
            logger.info(f"Successfully retrieved {len(user_items)} users from group {group_id}")
