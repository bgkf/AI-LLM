...
def _log_to_dict(log) -> dict:
    ...
    if log.actor:
        actor = {
            "id": log.actor.id,
            "type": _safe(log.actor.type),
            "alternateId": log.actor.alternate_id,
            "displayName": log.actor.display_name,
        }
    outcome = None
    if log.outcome:
        outcome = {
            "result": _safe(log.outcome.result),
            "reason": log.outcome.reason,
        }
    return {
        "published": _safe(log.published),
        "eventType": log.event_type,
        "displayMessage": log.display_message,
        "severity": _safe(log.severity),
        "actor": actor,
        "outcome": outcome,
    }
...
async def get_logs(
    ...
    try:
        ...
        log_items = [_log_to_dict(log) for log in logs]
        ...
        if fetch_all and response and hasattr(response, "has_next") and response.has_next():
            ...
            all_log_items = [_log_to_dict(log) for log in all_logs]

            logger.info(
                f"Successfully retrieved {len(all_log_items)} log entries across {pagination_info['pages_fetched']} pages"
            )
            return create_paginated_response(all_log_items, response, fetch_all_used=True, pagination_info=pagination_info)
        else:
