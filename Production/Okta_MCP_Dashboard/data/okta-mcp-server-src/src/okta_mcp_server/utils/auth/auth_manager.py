...
class OktaAuthManager:
    ...
    async def is_valid_token(self, expiry_duration: int = 3600) -> bool:
        ...
        # token_timestamp resets to 0 on each new process — if keyring has a token,
        # try to refresh it (if refresh token exists) or treat it as valid for this session.
        if api_token and self.token_timestamp == 0:
            if keyring.get_password(SERVICE_NAME, "refresh_token"):
                logger.info("New process with cached token — attempting refresh")
                if self.refresh_access_token():
                    return True
            logger.info("New process with cached token — treating as valid for this session")
            self.token_timestamp = int(time.time())
            return True
