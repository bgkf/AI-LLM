...
async function checkApi() {
  ...
  try {
    // Route the check through server.py so it uses the live Okta token.
    // /query with a minimal prompt — if the token is missing or invalid,
    // server.py returns a CLARIFY: message we can inspect without rendering.
    const res = await fetch("/query", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ prompt: "List all users with status ACTIVE [limit=1]" }),
    ...
    if (content.includes("401")) {
      setIndicator(elIndApi, elTooltipApi, "amber", "No token set — use Set Token");
      setTokenButtonState("unset");
    } else if (content.includes("CLARIFY") && content.includes("token")) {
      setIndicator(elIndApi, elTooltipApi, "amber", "No token set — use Set Token");
      setTokenButtonState("unset");
    } else if (content.startsWith("CLARIFY:")) {
      // Got a clarify response — server and Okta are both reachable,
      // the query just didn't match (shouldn't happen with this prompt).
      setIndicator(elIndApi, elTooltipApi, "green", "Okta API reachable · token valid");
    ...
    setIndicator(elIndApi, elTooltipApi, "red", "Could not reach Okta API");
    setTokenButtonState("error");
  }
}
...
function renderResults(parsed) {
  ...
  const isLogTool = tool_called === "get_audit_logs" || tool_called === "search_events";

  if (isLogTool) {
    elResultContent.innerHTML = buildLogTable(data);
  } else {
    elResultContent.innerHTML = buildGenericTable(data);
  }
...
function buildGenericTable(data) {
  ...
  const headerCols = keys.map(k =>
    `<th data-col="${escapeAttr(k)}">${escapeHtml(k)}</th>`
  ).join("");
  ...
  const rows = data.map((row, i) => {
    const cells = keys.map(k => {
      ...
      if (k === "lastLogin" || k === "created" || k === "lastUpdated") {
...
function buildLogTable(data) {
  const rows = data.map((event, i) => {
    ...
    return `
      ...
      <tr class="row-detail">
        <td colspan="5">${escapeHtml(raw)}</td>
      </tr>`;
  }).join("");
  ...
  return `
    <table class="result-table">
      <thead>
        <tr>
          <th class="cb-col"><input type="checkbox" id="cb-all" /></th>
          <th data-col="published">Published</th>
...
function openTokenModal() {
  elTokenInput.value = "";
  // Suppress password manager prompts (1Password, etc.)
  elTokenInput.setAttribute("autocomplete", "off");
  elTokenInput.setAttribute("data-lpignore", "true");
  elTokenInput.setAttribute("data-1p-ignore", "true");
...
elBtnTokenSave.addEventListener("click", async () => {
  ...
  try {
    ...
    elTokenInput.placeholder = "Failed to save — try again";
  }
});
